...
To allow the system to compile and run ad-hoc software, some packages need to be included, such as \textbf{bootget, gcc, XRT} and \textbf{acceleration}. The installation menu will open with the command
\vspace{0.1cm}
\begin{lstlisting}[language=bash]
...
\begin{figure}[h!t!]
	...
	\label{fig:prj2_3}
\end{figure}
After having installed all the needed packages, it is time rebuild the project with the command
\vspace{0.1cm}
\begin{lstlisting}[language=bash]
	petalinux-build
\end{lstlisting}
