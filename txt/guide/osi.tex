...
\begin{itemize}
	...
	\item \textbf{Filesystem Packages} $\rightarrow$ \textbf{base} $\rightarrow$ \textbf{util-linux} \textit{(optional, if available)}
	
	\item \textbf{Filesystem Packages} $\rightarrow$ \textbf{console} $\rightarrow$ \textbf{utils} $\rightarrow$ \textbf{git} $\rightarrow$ \textbf{[*]git}
	
	\item \textbf{Filesystem Packages} $\rightarrow$ \textbf{console} $\rightarrow$ \textbf{utils} $\rightarrow$ \textbf{strace}
...
\paragraph{Dropbear and OpenSSH}
Only one SSH server should be enabled in the root filesystem. The KR260 BSP
typically enables \textbf{Dropbear}, which is the recommended choice for this
guide because it provides a lightweight SSH server and integrates well with the
default PetaLinux configuration.

Enabling both \textbf{Dropbear} and \textbf{OpenSSH} at the same time may cause
the \texttt{do\_rootfs} task to fail due to package conflicts. Therefore, use
either Dropbear or OpenSSH, but never both simultaneously.

If OpenSSH is required (for example to provide a complete SFTP server), disable
Dropbear and enable the following packages:
\begin{itemize}
	\item \textbf{openssh}
	\item \textbf{openssh-sshd}
	\item \textbf{openssh-sftp-server}
\end{itemize}

If Dropbear is used, recent OpenSSH clients may attempt to use the SFTP
protocol internally when executing \texttt{scp}. If the target image does not
include an SFTP server, an error similar to

\begin{lstlisting}[language=bash]
	/usr/libexec/sftp-server: No such file or directory
\end{lstlisting}

may appear. In this case, force the legacy SCP protocol from the host machine:

\begin{lstlisting}[language=bash]
	scp -O file_name petalinux@board_ip:/home/petalinux/
\end{lstlisting}
